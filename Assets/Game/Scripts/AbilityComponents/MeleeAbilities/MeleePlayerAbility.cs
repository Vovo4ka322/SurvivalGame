...
namespace Game.Scripts.AbilityComponents.MeleeAbilities
{
    public class MeleePlayerAbility : AbilityUserBase<AbilitySet>, IAbilityUser
    {
        ...
        private void Awake()
        {
            AbilitiesDatas = new Dictionary<int, AbilitySet>
            {
                { FirstLevel, _abilityDataFirstLevel },
                { SecondLevel, _abilityDataSecondLevel },
                { ThirdLevel, _abilityDataThirdLevel },
            };
        }
        ...
        {
            TryUpgradeAbility(ref _counterForBladeFury, FirstLevel, SecondLevel, ThirdLevel, data => data.BladeFury,
                _bladeFury.ReplaceValue, BladeFuryUpgraded);
        }

        public override void UpgradeSecondAbility()
        {
            TryUpgradeAbility(ref _counterForBorrowedTime, FirstLevel, SecondLevel, ThirdLevel,
                data => data.BorrowedTime, _borrowedTime.ReplaceValue, BorrowedTimeIUpgraded);
        }

        public override void UpgradeThirdAbility()
        {
            TryUpgradeAbility(ref _counterForBloodLust, FirstLevel, SecondLevel, ThirdLevel, data => data.BloodLust,
                scriptableObject => _player.UpgradeCharacteristikByBloodLust(scriptableObject), BloodLustIUpgraded);
        }
    }
}