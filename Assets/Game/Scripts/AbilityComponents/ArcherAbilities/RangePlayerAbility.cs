...
namespace Game.Scripts.AbilityComponents.ArcherAbilities
{
    public class RangePlayerAbility : AbilityUserBase<AbilitySet>, IAbilityUser
    {
        ...
        {
            TryUpgradeAbility(ref _counterForMultiShot, FirstLevel, SecondLevel, ThirdLevel, data => data.MultiShot,
                _multiShot.ReplaceValue, MultiShotUpgraded);
        }

        public override void UpgradeSecondAbility()
        {
            TryUpgradeAbility(ref _counterForInsatiableHunger, FirstLevel, SecondLevel, ThirdLevel,
                data => data.InsatiableHunger, _insatiableHunger.ReplaceValue, InsatiableHungerUpgraded);
        }

        public override void UpgradeThirdAbility()
        {
            TryUpgradeAbility(ref _counterForBlur, FirstLevel, SecondLevel, ThirdLevel, data => data.Blur,
                scriptableObject => _player.SetEvasion(scriptableObject), BlurUpgraded);
        }
    }
}