...
namespace Game.Scripts.AbilityComponents.AbilityUpgrades
{
    ...
    {
        public void UpdateUpgradeDisplay(UpgradeTextDisplay display, int currentLevel, Func<AbilitySet, float> valueSelector, Func<int, AbilitySet> getAbilityDataForLevel)
        {
            if (currentLevel == 0)
            {
