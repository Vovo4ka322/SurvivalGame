                    ...
                    switch (config.NameAbilityType)
                    {
                            ...
                            foreach (FieldDisplayConfig field in config.FieldDisplays)
                            {
                                switch (field.Property)
                                {
                                    case NameAbilityProperty.Duration:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display, melee.CurrentBladeFuryLevel,
                                            data => data.BladeFury.Duration, melee.GetAbilityDataForLevel);

                                        break;
                                    case NameAbilityProperty.Cooldown:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display, melee.CurrentBladeFuryLevel,
                                            data => data.BladeFury.CooldownTime, melee.GetAbilityDataForLevel);
                        ...
                        case NameAbilityType.BorrowedTime:
                            ...
                            foreach (FieldDisplayConfig field in config.FieldDisplays)
                            {
                                ...
                                {
                                    case NameAbilityProperty.Duration:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display,
                                            melee.CurrentBorrowedTimeLevel, data => data.BorrowedTime.Duration,
                                            melee.GetAbilityDataForLevel);

                                        break;
                                    case NameAbilityProperty.Cooldown:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display,
                                            melee.CurrentBorrowedTimeLevel, data => data.BorrowedTime.CooldownTime,
                                            melee.GetAbilityDataForLevel);
                            ...
                            foreach (FieldDisplayConfig field in config.FieldDisplays)
                            {
                                switch (field.Property)
                                {
                                    case NameAbilityProperty.MovementSpeed:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display, melee.CurrentBloodLustLevel,
                                            data => data.BloodLust.MovementSpeed, melee.GetAbilityDataForLevel);

                                        break;
                                    case NameAbilityProperty.AttackSpeed:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display, melee.CurrentBloodLustLevel,
                                            data => data.BloodLust.AttackSpeed, melee.GetAbilityDataForLevel);
                    ...
                    switch (config.NameAbilityType)
                    {
                        case NameAbilityType.MultiShot:
                            ...
                            foreach (FieldDisplayConfig field in config.FieldDisplays)
                            {
                                ...
                                {
                                    case NameAbilityProperty.Duration:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display, range.CurrentMultiShotLevel,
                                            data => data.MultiShot.Duration, range.GetAbilityDataForLevel);

                                        break;
                                    case NameAbilityProperty.Cooldown:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display, range.CurrentMultiShotLevel,
                                            data => data.MultiShot.CooldownTime, range.GetAbilityDataForLevel);

                                        break;
                                    case NameAbilityProperty.ArrowCount:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display, range.CurrentMultiShotLevel,
                                            data => data.MultiShot.ArrowCount, range.GetAbilityDataForLevel);
                        ...
                        case NameAbilityType.InsatiableHunger:
                            ...
                            foreach (FieldDisplayConfig field in config.FieldDisplays)
                            {
                                switch (field.Property)
                                {
                                    case NameAbilityProperty.Duration:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display,
                                            range.CurrentInsatiableHunger, data => data.InsatiableHunger.Duration,
                                            range.GetAbilityDataForLevel);

                                        break;
                                    case NameAbilityProperty.Cooldown:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display,
                                            range.CurrentInsatiableHunger, data => data.InsatiableHunger.CooldownTime,
                                            range.GetAbilityDataForLevel);

                                        break;
                                    case NameAbilityProperty.Vampirism:
                                        _upgradeHelper.UpdateUpgradeDisplay(field.Display,
                                            range.CurrentInsatiableHunger, data => data.InsatiableHunger.Vampirism,
                                            range.GetAbilityDataForLevel);
                        ...
                        case NameAbilityType.Blur:
                            ...
                            foreach (FieldDisplayConfig field in config.FieldDisplays)
                            {
                                ...
                                {
                                    _upgradeHelper.UpdateUpgradeDisplay(field.Display, range.CurrentBlurLevel,
                                        data => data.Blur.Evasion, range.GetAbilityDataForLevel);
                                }
                            }
