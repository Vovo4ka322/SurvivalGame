...
namespace Game.Scripts.AbilityComponents
{
    public abstract class AbilityUserBase<TAbilityData> : MonoBehaviour, IAbilityUser
    {
        ...
        protected void TryUpgradeAbility<TScriptableObject>(
            ref int counter,
            int level1,
            int level2,
            int level3,
            Func<TAbilityData, TScriptableObject> getScriptableObject,
            Action<TScriptableObject> upgradeAction,
            Action upgradedEvent)
        {
        ...
        private void Upgrade<TScriptableObject>(ref int counter, int level,
            Func<TAbilityData, TScriptableObject> getScriptableObject, Action<TScriptableObject> upgradeAction,
            Action upgradedEvent)
        {
